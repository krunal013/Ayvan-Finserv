...
function App() {
    ...
    const fetchQrCode = async (clientId) => {
        try {
            const response = await fetch(`http://localhost:5001/api/whatsapp/qr-code?clientId=${encodeURIComponent(clientId)}`);
    ...
    useEffect(() => {
        if (user) {
            ...
            const checkReadyStatus = async () => {
                try {
                    const response = await fetch(`http://localhost:5001/api/whatsapp/status?clientId=${encodeURIComponent(user.name)}`);
                    const jsonData = await response.json();
                    if (jsonData.isReady) {
                        setIsReady(true);
    ...
    const handleLogout = async () => {
        try {
            await fetch(`http://localhost:5001/api/whatsapp/logout?clientId=${encodeURIComponent(user.name)}`, { method: 'POST' });
            setQrCodeUrl('');
            setIsReady(false);
            fetchQrCode(user.name);
        } catch (error) {
    ...
    const handleFileUpload = async (event) => {
        ...
        try {
            const response = await fetch(`http://localhost:5001/api/whatsapp/upload?clientId=${encodeURIComponent(user.name)}`, {
                method: 'POST',
                body: formData
            });
